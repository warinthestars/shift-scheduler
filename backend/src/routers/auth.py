...
async def login(request: LoginRequest, db: AsyncSession = Depends(get_db)):
    """Local email and password login returning JWT access token"""
    result = await db.execute(select(User).where(User.email == request.email))
...
async def firebase_login(request: FirebaseLoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    - If USE_MOCK_FIREBASE=true, resolves mock tokens (e.g. 'mock-firebase-token-123')
      to a local demo worker user.
    - If USE_MOCK_FIREBASE=false, validates real Firebase ID token and provisions/syncs user.
    """
